...
export function applyCornerWingCarcassCeiling(
  ...
  if (metrics.__wingIsUnifiedCabinet) {
    ...
    const topW = Math.max(0.05, activeWidth - __leftInsetX - __rightInsetX - 0.001);
  ...
  for (const cell of cornerCells) {
    ...
    const __hz = resolveCornerWingHorizPlacement(params, metrics, cellD, 0.05);
    const __h = Math.max(woodThick * 2, cell.bodyHeight);

    const __cellW = Math.max(0.05, cell.width);
    ...
    const topW = Math.max(0.05, __cellW - __leftInsetX - __rightInsetX - 0.001);
