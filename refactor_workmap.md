...
> עדכון Stage 74 — 3 במאי 2026:
> זהו שלב תכנוני/בקרתי ולא עוד פירוק אוטומטי. נוצר `docs/REFACTOR_NEXT_STAGE_PLAN.md` כ־decision gate מקצועי להמשך: לא מפרקים קבצים לפי מספר שורות בלבד, אלא רק כאשר יש ערבוב אחריות אמיתי, חוזה ציבורי יציב, guard קונקרטי, מסלול אימות ברור ותועלת תחזוקתית אמיתית. התוכנית מגדירה את המועמדים הראויים הבאים Stage 75–80 כ־review stages — sketch box fronts/doors, drawer/shared render contracts, sketch-box UI controls, runtime access surfaces, Order PDF flows, וסגירת מדידות/ביצועים — ומעגנת stop conditions כדי לעצור refactor כשאין בו תועלת אמיתית. השלב מעוגן ב־guard ייעודי וב־control plane כדי שלא נחזור ל"פיצול לפי סרגל".

> עדכון Stage 75 — 3 במאי 2026:
> בוצע review מקצועי לאזור sketch box fronts/doors לפי שער Stage 74. לא פורקו כל קבצי fronts; רק seam אחד שהוכיח ערבוב אחריות אמיתי: `render_interior_sketch_boxes_fronts_door_visuals.ts` נשאר orchestration ציבורי, בעוד mirror/material resolution עבר ל־`render_interior_sketch_boxes_fronts_door_visual_materials.ts`, החלטת special/styled/slab route עברה ל־`render_interior_sketch_boxes_fronts_door_visual_routes.ts`, ויצירת visual/mesh + pick metadata עברה ל־`render_interior_sketch_boxes_fronts_door_visual_core.ts`. trims ו־classic accents נשארו בבעלים קיימים. השלב מעוגן ב־guard ייעודי ומונע מצרכנים לעקוף את facade ה־visual.

> עדכון Stage 76 — 3 במאי 2026:
> בוצע review מקצועי ל־drawer/shared render contracts לפי שער Stage 74. לא פורקו `render_drawer_ops_external.ts` או `render_drawer_ops_internal.ts` שוב, כי שם לא הייתה הצדקה מספקת לפיצול נוסף. במקום זאת פוצל seam אמיתי בתוך `render_drawer_ops_shared.ts`: contracts עברו ל־`render_drawer_ops_shared_types.ts`, runtime guards ל־`render_drawer_ops_shared_guards.ts`, argument/callback readers ל־`render_drawer_ops_shared_readers.ts`, drawer-op parsing ל־`render_drawer_ops_shared_ops.ts`, ו־mirror/glass/curtain visual-state policy ל־`render_drawer_ops_shared_visual_state.ts`. ה־shared public facade נשאר יציב, ו־external/internal drawer owners ממשיכים לייבא רק ממנו.

> עדכון Stage 77 — 3 במאי 2026:
> בוצע review מקצועי לאזור UI sketch-box controls לפי שער Stage 74. לא הוזזו מקטעי JSX רק כדי להקטין קובץ; פוצל seam אמיתי סביב runtime/control ownership. `interior_layout_sketch_box_controls_runtime.ts` נשאר facade ציבורי יציב, בעוד shared types, sync helpers, dimension draft commits, panel/tool toggles, base/leg controls ו־cornice selection עברו לבעלי־אחריות ממוקדים. בנוסף, active-tool view-state derivation עבר ל־`interior_layout_sketch_box_controls_state.ts`, ו־option data עבר ל־`interior_layout_sketch_box_controls_options.ts`, כך שה־section נשאר ממוקד ברינדור ובקריאות ל־facade הציבורי. השלב מעוגן ב־guard ייעודי ובבדיקות UI/source רלוונטיות.
