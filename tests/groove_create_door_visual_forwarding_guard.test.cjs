...
test('createDoorVisual wrappers forward groovePartId through builder paths', () => {
  ...
  assert.match(
    renderDoorOps,
    /readDoorVisualFactory[\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options[\s\S]*?value\([\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options/,
  ...
  assert.match(
    moduleLoopShared,
    /const createDoorVisual:[\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options[\s\S]*?Reflect\.apply\([\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options/,
  ...
  const renderDrawerOps = [
    ...
    read('esm/native/builder/render_drawer_ops_shared_readers.ts'),
    read('esm/native/builder/render_drawer_ops_external.ts'),
  ].join('\n');
  assert.match(
    renderDrawerOps,
    /readCreateDoorVisual[\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options[\s\S]*?value\([\s\S]*?mirrorLayout,[\s\S]*?groovePartId,[\s\S]*?options/,
    'render_drawer_ops.ts must forward groovePartId to createDoorVisual()'
  );

  assert.match(
    renderDrawerOps,
    /createDoorVisual\([\s\S]*?globalFrontMat,[\s\S]*?1,[\s\S]*?false,[\s\S]*?null,[\s\S]*?partId(?:,\s*[\s\S]*?)?\s*\)/,
